...
class iRelationsExtractor:
    ...
    def extract_relations(self, 
        ...
        while tries < max_tries:
            ...
            """
            # 在抛出错误之前保存 relationships 到 md 文件
            try:
                with open('relationships_debug.md', 'w', encoding='utf-8') as f:
                    f.write("# Relationships Extraction Debug Information\n\n")
                    f.write("## Current Relationships Data\n")
                    f.write(f"```json\n{str(relationships)}\n```\n\n")
                    f.write("## Error Details\n")
                    if not relationships:
                        f.write("- relationships 对象为空\n")
                    else:
                        f.write("- relationships 对象中没有 'relationships' 键\n")
                    # 导入datetime模块并获取当前时间戳
        ...
        if relationships and "relationships" in relationships:
            ...
            for relationship in relationships["relationships"]:
                print(f"[DEBUG] Processing relationship: {relationship}")  # 添加调试信息
                if "name" in relationship:
                    ...
                    if startEntity_in_input_entities is not None and endEntity_in_input_entities is not None :
                        ...
                        if startEntity is None or endEntity is None:
                            print(f"[WARNING] Skipping relationship due to unmatched entity: {startEntity} -> {endEntity}")
                            continue
                    ...
                    print(f"[WARNING] Skipping relationship due to missing 'name': {relationship}")
